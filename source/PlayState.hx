...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Scrapped', 0.2], //From 0% to 19%
		['Rubbish', 0.4], //From 20% to 39%
		['Troublesome', 0.5], //From 40% to 49%
		['Lazybones', 0.6], //From 50% to 59%
		['Pah!', 0.69], //From 60% to 68%
		['Reliable', 0.7], //69%
		['Splendid', 0.8], //From 70% to 79%
		['Grand', 0.9], //From 80% to 89%
		['Really Useful!', 1], //From 90% to 99%
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'puffball':
					ob1 = new FlxSprite(-701,-488).loadGraphic(Paths.image('bgs/puffball/puffballstage'));
					ob1.antialiasing = ClientPrefs.globalAntialiasing;
					ob1.setGraphicSize(2700);
					ob1.updateHitbox();
					add(ob1);
				case 'harbor':
					//var ref:FlxSprite = new FlxSprite(-1264,-921).loadGraphic(Paths.image('bgs/harbor/ref'));
					//ref.antialiasing = ClientPrefs.globalAntialiasing;
					//ref.alpha = 0.4;
					//ref.setGraphicSize(3515);
					//ref.updateHitbox();
					//overlaySprs.push(ref);
	
					ob2 = new FlxSprite(-1254,-912).loadGraphic(Paths.image('bgs/harbor/sky'));
					ob2.antialiasing = ClientPrefs.globalAntialiasing;
					ob2.setGraphicSize(3500);
					ob2.updateHitbox();
					add(ob2);
	
					ob3 = new FlxSprite(-1254,-912).loadGraphic(Paths.image('bgs/harbor/clouds'));
					ob3.antialiasing = ClientPrefs.globalAntialiasing;
					ob3.setGraphicSize(3500);
					ob3.updateHitbox();
					add(ob3);
	
					ob1 = new FlxSprite(-1254,-912).loadGraphic(Paths.image('bgs/harbor/back'));
					ob1.antialiasing = ClientPrefs.globalAntialiasing;
					...
					ob4.frames = Paths.getSparrowAtlas('bgs/harbor/wata');
		...
		switch (SONG.song)
		{
			case 'confusion and delay':
				...
				mech1.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override public function update(elapsed:Float)
	{
		if (FlxG.keys.pressed.SHIFT && editable)
			...
			{
				var compEdits:String = '';
				for (i in [ob1,ob2,ob3,ob4,ob5,ob6,ob7,ob8,ob9,ob10,ob11,ob12,ob13,ob14,ob15,ob16])
				{
					if (i != null)
					compEdits += '${i}\n';
				}
		...
		else if (FlxG.keys.justPressed.SPACE && editable)
				{
					var stupid:Array<FlxSprite> = [ob1,ob2,ob3,ob4,ob5,ob6,ob7,ob8,ob9,ob10,ob11,ob12,ob13,ob14,ob15,ob16];
					curEditSprite++;
					if (curEditSprite >= stupid.length)
						curEditSprite = 0;
					if (curEditSprite < 0)
						curEditSprite = stupid.length - 1;

					while (stupid[curEditSprite] == null)
					{
						curEditSprite++;
						if (curEditSprite >= stupid.length)
							curEditSprite = 0;
					...
					editbleSprite = stupid[curEditSprite];
				}
		else if (FlxG.keys.justPressed.V && editable)
				{
					FlxTween.color(editbleSprite, 1, FlxColor.BLUE, FlxColor.WHITE);
				}
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			...
			if(strumsBlocked[key] != true && spr != null && spr.animation.curAnim.name != 'confirm' && !blockInput)
			{
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				&& daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit && !daNote.blockHit && !blockInput) {
					goodNoteHit(daNote);
				}
			});
